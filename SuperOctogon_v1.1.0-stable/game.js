...
function startMusic() {
    ...
    let sequence = [220, 261, 293, 329, 392, 329, 293, 261];
    let step = 0;
    ...
    function playStep() {
        ...
        let note = sequence[step % sequence.length];
        if (step % 4 === 0) playTone(note / 2, 'sawtooth', 0.4, 0.08); 
        playTone(note * (spd || 1), 'square', 0.15, 0.03); 
        step++;
        setTimeout(playStep, 200 / (spd || 1));
    }
    playStep();
}
...
window.addEventListener('keydown', e => {
    ...
    keys[k] = true;
    if (userClickedToStart) initAudio();
...
function startG() {
    menuOverlay.style.display = 'none'; verStamp.style.display = 'none'; hud.style.display = 'flex'; state = 'PLAYING';
...
function go() {
    state = 'GAMEOVER'; hud.style.display = 'none'; menuOverlay.style.display = 'flex'; verStamp.style.display = 'block';
...
function loop(timestamp) {
    ...
    if (state === 'BOOT' && userClickedToStart) {
        bootTimer += 16.666 * dt;
        if (bootTimer < 2000) {
            wRot += 0.002 * dt;
        } else if (bootTimer >= 2000 && bootTimer < 2100) {
            creatorSplash.classList.add('hidden');
            gameLoading.classList.remove('hidden');
        } else if (bootTimer >= 2100 && loadingProgress < 100) {
            loadingProgress += 1.2 * dt;
